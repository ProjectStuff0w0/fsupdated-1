...
def verifier():
    ...
    if not dd.get() or dd.get() == "SELECT":
        txt.insert(INSERT,"Select Sem Properly\n")
        d=1
...
def view_student() :
    ...
    txt.delete(1.0,END)
    txt.insert(INSERT,"\n\tUSN\t\tStudent Name\t\t\tSem\t\tSection\t\tFather Name\t\t Mobile\t\tAddress\n")
    for key, value in return_data.items() :
...
def classwise() :
   ...
   #txt.insert(INSERT,"\nRoll No.\tFirst Name\tLastName\tClass\tSection\tGender\tFather\tMother\tMobile\tAddress")
    if combo_class.get() in ('1', '2', '3', '4', '5','6','7','8') :
        for key, value in return_data.items() :
            if return_data[key] == "This Record is Deleted from System" :
                continue
            else :
...
if __name__=="__main__":
    ...
    aa = ttk.Combobox(root,width=10)    # Year entry
    ...
    dd = ttk.Combobox(root,width=10)    # Sem name
    dd['values']= ("SELECT",1, 2, 3, 4, 5,6,7,8)
    ...
    label_roll=Label(root,text="USN :",font=("Arial",10),width=10)
    label_a1=Label(root,text="Year :",font=("Arial",10),width=10)
    label_b1=Label(root,text="First Name :",font=("Arial",10),width=10)
    label_c1=Label(root,text="Last Name :",font=("Arial",10),width=10)
    label_d1=Label(root,text="Sem :",font=("Arial",10),width=10)
    ...
    aa1 = ttk.Combobox(root,width=10)    # Year entry
    ...
    dd1 = ttk.Combobox(root,width=10)    # Sem name
    dd1['values']= ("SELECT",1,2,3,4,5,6,7,8)
    ...
    label_delete=Label(root,text="USN:",font=("Arial",12),width=10)
    ...
    dashboard=Label(root,text="University Student Management",font=("Arial",16),foreground="#ee9a4d")
    ...
    combo_class['values']= (1, 2, 3, 4, 5,6,7,8)
